...
class nixlBackendEngine {
    ...
    protected:
        ...
        [[nodiscard]] nixlStatus setInitParam(const std::string &key, const std::string &value) {
        ...
        [[nodiscard]] nixlStatus getInitParam(const std::string &key, std::string &value) const {
    ...
    public:
        ...
        const nixlBackend& getType() const noexcept { return backendType; }
        const nixlBParams& getCustomParams() const noexcept { return customParams; }
        ...
        virtual nixlMemList getSupportedMems() const = 0;  // TODO: Return by const-reference and mark noexcept?


        // *** Pure virtual methods that need to be implemented by any backend *** //

        // Register and deregister local memory
        virtual nixlStatus registerMem (const nixlBlobDesc &mem,
                                           const nixlMemType &nixl_mem,
                                           nixlBackendMD* &out) = 0;
        virtual nixlStatus deregisterMem (nixlBackendMD* meta) = 0;
        ...
        virtual nixlStatus connect(const std::string &remote_agent) = 0;
        virtual nixlStatus disconnect(const std::string &remote_agent) = 0;

        // Remove loaded local or remtoe metadata for target
        virtual nixlStatus unloadMD (nixlBackendMD* input) = 0;

        // Preparing a request, which populates the async handle as desired
        virtual nixlStatus prepXfer (const nixlXferOp &operation,
        ...
        virtual nixlStatus estimateXferCost(const nixlXferOp &operation,
                                               const nixl_meta_dlist_t &local,
                                               const nixl_meta_dlist_t &remote,
                                               const std::string &remote_agent,
        ...
        virtual nixlStatus postXfer (const nixlXferOp &operation,
                                        const nixl_meta_dlist_t &local,
                                        const nixl_meta_dlist_t &remote,
                                        const std::string &remote_agent,
                                        nixlBackendReqH* &handle,
        ...
        virtual nixlStatus loadRemoteConnInfo (const std::string &remote_agent,
                                                  const std::string &remote_conn_info) {
            return NIXL_ERR_BACKEND;
        }

        // Load remtoe metadata, if supported.
        virtual nixlStatus loadRemoteMD (const nixlBlobDesc &input,
                                            const nixlMemType &nixl_mem,
                                            const std::string &remote_agent,
        ...
        virtual nixlStatus loadLocalMD (nixlBackendMD* input,
                                           nixlBackendMD* &output) {
            return NIXL_ERR_BACKEND;
        }
        ...
        virtual nixlStatus getNotifs(notif_list_t &notif_list) { return NIXL_ERR_BACKEND; }

        // Generates a standalone notification, not bound to a transfer.
        virtual nixlStatus genNotif(const std::string &remote_agent, const std::string &msg) const {
            return NIXL_ERR_BACKEND;
        }

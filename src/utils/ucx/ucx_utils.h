...
public:
    ...
    nixlUcxContext(std::vector<std::string> devices,
                   ...
                   bool prog_thread, unsigned long num_workers, nixl_thread_sync_t sync_mode);
    ~nixlUcxContext();
...
class nixlUcxWorker {
  public:
    explicit nixlUcxWorker(const nixlUcxContext&, ucp_err_handling_mode_t ucp_err_handling_mode = UCP_ERR_HANDLING_MODE_NONE);
...
private:
    ...
    [[nodiscard]] static ucp_worker* createUcpWorker(const nixlUcxContext&);
};

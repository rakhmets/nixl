...
public:
    ...
    /* Ser/Des for Strings */
    nixlStatus addStr(const std::string &tag, const std::string &str);
    std::string getStr(const std::string &tag);

    /* Ser/Des for Byte buffers */
    nixlStatus addBuf(const std::string &tag, const void* buf, ssize_t len);
    ssize_t getBufLen(const std::string &tag) const;
    nixlStatus getBuf(const std::string &tag, void *buf, ssize_t len);

    /* Ser/Des buffer management */
    std::string exportStr() const;
    nixlStatus importStr(const std::string &sdbuf);
